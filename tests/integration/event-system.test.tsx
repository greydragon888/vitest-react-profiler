...
describe("Event System - Integration Tests", () => {
  ...
  describe("Performance", () => {
    it("Async operations complete in < 20ms", async () => {
      const TestComponent: FC<{ value: number }> = ({ value }) => (
        <div>{value}</div>
      );
      const ProfiledComponent = withProfiler(TestComponent);
      ...
      // Measure waitForNextRender performance
      ...
      // Should complete in < 20ms (excluding render time)
      expect(elapsed).toBeLessThan(20);

      // Measure onRender callback performance
      const listener = vi.fn();
      ...
      // Event notification should be near-instant (< 5ms)
      expect(elapsed2).toBeLessThan(20);
    });

    it("Memory usage with many listeners", () => {
      const TestComponent: FC<{ value: number }> = ({ value }) => (
        <div>{value}</div>
      );
      const ProfiledComponent = withProfiler(TestComponent);
      ...
      // All 100 listeners should be called
      listeners.forEach((listener) => {
        expect(listener).toHaveBeenCalledTimes(1);
      });

      // Should complete in reasonable time even with 100 listeners (< 50ms)
      expect(elapsed).toBeLessThan(50);

      // Memory check: Multiple renders shouldn't accumulate memory
      for (let i = 3; i <= 20; i++) {
        rerender(<ProfiledComponent value={i} />);
      }

      // All listeners called 18 additional times
      listeners.forEach((listener) => {
        expect(listener).toHaveBeenCalledTimes(19); // 1 + 18
      });
    });

    it("Performance with large render history (1000+ renders)", async () => {
      const TestComponent: FC<{ value: number }> = ({ value }) => (
        <div>{value}</div>
      );
      const ProfiledComponent = withProfiler(TestComponent);
      ...
      // Event operations should still be fast with large history
      const listener = vi.fn();
      ...
      // Should complete quickly even with 1000+ render history (< 20ms)
      expect(elapsed).toBeLessThan(50);

      // waitForNextRender should also be fast
      const start2 = performance.now();
      const promise = ProfiledComponent.waitForNextRender();

      rerender(<ProfiledComponent value={1001} />);
